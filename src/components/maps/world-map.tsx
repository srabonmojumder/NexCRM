  ...
  useEffect(() => {
    ...
    clients.forEach((client) => {
      ...
      L.marker([client.location.lat, client.location.lng], {
        ...
        .bindPopup(
          `<div style="font-family:Inter,system-ui,sans-serif;color:#0a0e1a;padding:2px 4px;">
            <div style="font-weight:700;font-size:13px;">${client.company}</div>
            <div style="font-size:11px;color:#6b7280;margin-top:2px;">${client.location.city}, ${client.location.country}</div>
            <div style="font-size:11px;color:#3a60ff;font-weight:600;margin-top:4px;">$${client.mrr.toLocaleString()} MRR</div>
          </div>`,
          { closeButton: false }
        )
        .addTo(m);
